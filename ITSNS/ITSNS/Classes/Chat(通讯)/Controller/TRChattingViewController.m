...
- (IBAction)sendAction:(id)sender {
    ...
    //去掉字符串两头的空格
    self.commentTV.text = [self.commentTV.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    ...
    if (self.selectedImageViews.count>0) {
        ...
        [UIView animateWithDuration:.5 animations:^{
          self.addImageButton.left = LYMargin;
        }];
...
-(void)keyboardChangeAction:(NSNotification *)noti{
    ...
    if (keyboardF.origin.y==LYSH) {
        ...
        self.tableView.height = LYSH;
    }else{//软键盘弹出
        
        self.commentBar.transform = CGAffineTransformMakeTranslation(0, -keyboardF.size.height);
        //tableView高度变短
        self.tableView.height = LYSH-keyboardF.size.height-self.commentBar.height;
        //显示最新消息
        [self showNewMessage];
    }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch ((int)msgBody.messageBodyType) {
        case eMessageBodyType_Text:
        {
         ...
         h = tv.textLayout.textBoundingSize.height + 30;
