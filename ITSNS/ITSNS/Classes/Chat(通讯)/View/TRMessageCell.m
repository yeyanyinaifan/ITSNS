...
-(void)setMessage:(EMMessage *)message{
    ...
    if ([message.from isEqualToString:self.toUser.username]) {//对方发的
        ...
        self.headIV.left = LYSW-self.headIV.width-LYMargin;
    ...
    switch ((int)msgBody.messageBodyType) {
        ...
        {
            //把图片和音频控件隐藏
            self.messageIV.hidden = self.voiceView.hidden  = YES;
//            把文本控件显示
            ...
            if ([message.to isEqualToString:self.toUser.username]) {//自己的
                ...
                //显示到右边
                self.paopaoIV.left = LYSW-self.headIV.width-LYMargin-self.paopaoIV.width;
                
                
            }else{
               
                //显示到左边
                self.paopaoIV.left = self.headIV.width+LYMargin;
            }
        ...
        case eMessageBodyType_Voice:
        {
            ...
            self.textView.hidden = self.messageIV.hidden = YES;
            //设置显示时间
